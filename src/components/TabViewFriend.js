...
export default function TabViewExample({ data, bindAcceptInvitation, bindDeletedInvitation }) {
        ...
        .map((friendship) => {
          return (
            <View style={styles.itemList}>
              ...
              <Feather style={styles.icons} name="map-pin" size={22} color="white" />
  ...
  const SecondRoute = () => (
    <View style={{ flex: 1 }}>
      {data
        .filter((friendship) => !friendship.validated && !friendship.isInvited)
        .map((friendship) => {
          return (
            <View style={styles.itemList}>
              <View style={styles.itemListContent}>
    ...
    <ScrollView style={{}}>
      <View style={{ flex: 1 }}>
        <TabView
          style={{
            height:
              heightTab1 >= heightTab2
                ? heightTab1 + TAB_HEIGHT + TAB_BAR_HEIGHT
                : heightTab2 + TAB_HEIGHT + TAB_BAR_HEIGHT,
          }}
